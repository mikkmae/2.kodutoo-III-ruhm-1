<?php 
	...
	function addCarPlate($car_plate, $car_color, $carname) {
		
		$mysqli = new mysqli($GLOBALS["servername"], $GLOBALS["server_username"], $GLOBALS["server_password"], $GLOBALS["database"]);
		
		$stmt = $mysqli->prepare("INSERT INTO car_plates (user_id, number_plate, color, carname) VALUES (?,?,?,?)");
		$stmt->bind_param("isss", $_SESSION["logged_in_user_id"], $car_plate, $car_color, $carname);
